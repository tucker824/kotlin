    ...
    private fun process(irClass: IrClass) {
        ...
        if (privateInstanceField != publicInstanceField) {
            ...
            pendingTransformations.add {
                (privateInstanceField.parent as IrDeclarationContainer).declarations.add(privateInstanceField)
            }
        } else {
            with(context.createIrBuilder(publicInstanceField.symbol)) {
        ...
        pendingTransformations.add {
            (publicInstanceField.parent as IrDeclarationContainer).declarations.add(publicInstanceField)
        }
    }
}