...
import org.jetbrains.kotlin.fir.expressions.impl.FirArgumentListWithResolutionInfo
...
object FirJavaGenericVarianceViolationTypeChecker : FirFunctionCallChecker() {
    ...
    override fun check(expression: FirFunctionCall, context: CheckerContext, reporter: DiagnosticReporter) {
        ...
        if (calleeFunction.origin !in javaOrigin) {
            return
        }
        val argList = expression.argumentList as? FirArgumentListWithResolutionInfo
        val typeParameterSubstitutor = argList?.typeParameterSubstitutor ?: return
        for ((arg, param) in argList.mapping) {
            ...
            if (!AbstractTypeChecker.isSubtypeOf(typeCtx, argType, lowerBound)) {
