...
class HLImportMemberIntention :
    AbstractHLIntention<KtNameReferenceExpression, HLImportMemberIntention.Input>(KtNameReferenceExpression::class), HighPriorityAction {
    override val applicabilityRange: HLApplicabilityRange<KtNameReferenceExpression> get() = ApplicabilityRanges.SELF

    override val applicator: HLApplicator<KtNameReferenceExpression, Input> get() = Companion.applicator
    ...
    override val inputProvider: HLApplicatorInputProvider<KtNameReferenceExpression, Input> = inputProvider { psi ->
        ...
        when (symbol) {
            ...
            is KtCallableSymbol -> {
                val callableId = symbol.callableIdIfNonLocal ?: return@inputProvider null
                if (callableId.callableName.isSpecial) return@inputProvider null
                if (!canBeImported(symbol)) return@inputProvider null
                val shortenCommand = collectPossibleReferenceShortenings(
                    psi.containingKtFile,
            ...
            else -> return@inputProvider null
        }
    }
    ...
    companion object {
        val applicator = applicator<KtNameReferenceExpression, Input> {
            ...
            isApplicableByPsi {
                // Ignore simple name expressions
                if (it.getQualifiedElement() == it) return@isApplicableByPsi false
                if (it.isInImportDirective()) return@isApplicableByPsi false
                true
            }
            applyTo { _, input ->
